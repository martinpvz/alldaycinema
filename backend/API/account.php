    ...
    public function create($post){
        ...
        if(isset($post['name'])){
            ...
            if($result->num_rows == 0){
                ...
                //SE REALIZA LA CONSULTA PARA INSERTAR LOS DATOS EN LA TABLA DE CUENTA
                $sql = "SELECT idcuenta FROM cuentas WHERE nombre = '{$post['name']}'";
                if($result = $this->conexion->query($sql)){
                    ...
                    $sql = "INSERT INTO usuarios (idusuario, idcuenta, user, pass) VALUES (NULL, '$idcuenta','{$post['user']}','{$post['password']}')"; 
                    if ($this->conexion->query($sql)) {
                        $this->response['estatus'] =  "Correcto";
